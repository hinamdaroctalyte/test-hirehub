...
export default function Dropdown2({ options, setState, className, menuWidth, defaultTitle }) {
    const [selectedOption, setSelectedOption] = useState(defaultTitle ? defaultTitle : options[0]); // Set the default selected option
    ...
    return (
        <Menu as="div" className={`relative inline-block text-left ${className}`}>
            ...
            >
                <Menu.Items className={`absolute z-[2] right-0 mt-2 min-w-full ${menuWidth} origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none`}>
